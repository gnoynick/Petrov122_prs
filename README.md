...
## PR3: Использование элементов дизайна
...
## PR4: Навигация между страницами
...
- `app/src/main/res/navigation/nav_graph.xml` (подразумевается, нужно создать)
...
1. **Поток входа:** Вход → Главная (успех) / Регистрация (новый пользователь)
2. **Поток регистрации:** Регистрация → Главная (успех) / Вход (существующий пользователь)
3. **Поток выхода:** Главная → Вход
4. **Гостевой поток:** Вход → Главная (ограниченный доступ)
...
- **10 предопределенных подсказок:**
  - Поле идентификатора входа
  - Поле пароля входа
